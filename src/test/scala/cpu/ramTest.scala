...
object RAMTest {
    ...
    def loadSerial(tester: PeekPokeTester[CPUTest], c: CPUTest, fname: String) {
        ...
        tester.poke(c.io.init_serial, 1)
        for (i <- data.indices) {
            tester.poke(c.io.dd_serial, data(i))
            tester.step(1)
            //println(data(i))
        }
        tester.poke(c.io.init_serial, 0)
        tester.step(1)
    }
}